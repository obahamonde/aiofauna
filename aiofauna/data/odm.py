...
class FaunaModel(JSONModel):
    ...
    async def provision(cls) -> bool:
        ...
        try:
            if not await _q(q.exists(q.collection(cls.__name__.lower()))):  # type: ignore
                await _q(q.create_collection({"name": cls.__name__.lower()}))  # type: ignore

                cls.logger.info("Created collection %s", cls.__name__.lower())

            if not await _q(q.exists(q.index(cls.__name__.lower()))):  # type: ignore
                await _q(
                    q.create_index(  # type: ignore
                        {
                            "name": cls.__name__.lower(),
                            "source": q.collection(cls.__name__.lower()),  # type: ignore
                        }
                    )
                )
